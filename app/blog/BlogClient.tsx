...
function BlogClientContent({ posts }: { posts: PostClient[] }) {
  ...
  const postsBySeries = useMemo(() => {
    ...
    posts.forEach((post) => {
      const series = post.series || 'From Filter Coffee to German Bread'
      if (!grouped[series]) {
        grouped[series] = []
      }
  ...
  const allSeries = useMemo(() => {
    ...
    posts.forEach((post) => {
      seriesSet.add(post.series || 'From Filter Coffee to German Bread')
    })
  ...
  useEffect(() => {
    ...
    if (selectedSeries) {
      filtered = filtered.filter((post) => (post.series || 'From Filter Coffee to German Bread') === selectedSeries)
    }
    ...
    <div className="relative min-h-screen bg-[#faf9f7] dark:bg-[#1a1a1a] pt-16">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center mb-16 animate-fade-in">
          ...
          <h1 className="text-5xl sm:text-6xl font-serif font-bold text-[#1f2933] dark:text-[#e5e7eb] mb-4">
          ...
          {/* Series Filter (acts like a series tab bar) */}
          {allSeries.length > 1 && (
            <div className="flex flex-wrap gap-2 justify-center mb-6 border-b border-[#e2e8f0] dark:border-[#4a5568] pb-3">
              <Link
                href="/blog"
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                  !selectedSeries
                    ? 'bg-[#6b8e6b] text-white dark:bg-[#7a9a7a] shadow-sm'
        ...
        {filteredPosts.length === 0 ? (
          <div className="text-center py-20 animate-fade-in">
            <div className="mb-6 flex items-center justify-center">
              <IconDocument className="w-14 h-14 text-[#6b8e6b]" />
            </div>
                  ...
                  <div key={series} className="animate-fade-in">
                    <div className="mb-8 pb-4 border-b border-[#e2e8f0] dark:border-[#4a5568]">
                      ...
                      {seriesPosts.length > 0 && (
                        <div className="mt-4">
                          <p className="text-xs font-semibold uppercase tracking-wide text-[#718096] dark:text-[#9ca3af] mb-2 inline-flex items-center gap-2">
                          ...
                          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-3 mb-4">
                                ...
                                <time dateTime={post.date} className="text-sm text-[#718096] dark:text-[#9ca3af]">
                              ...
                              <div className="flex items-center justify-between mt-4">
                                <div className="flex items-center gap-4">
                                  ...
                                  <div className="flex gap-1">
                                    <IconBook className="w-4 h-4 text-[#718096] dark:text-[#9ca3af]" />
                                    <IconDocument className="w-4 h-4 text-[#718096] dark:text-[#9ca3af]" />
                                    <IconGlobe className="w-4 h-4 text-[#718096] dark:text-[#9ca3af]" />
                                  </div>
                                </div>
                              </div>
                            </div>
